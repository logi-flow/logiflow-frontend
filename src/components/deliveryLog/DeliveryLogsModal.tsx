...
interface Props {
  ...
  log: PageDto<GetDeliveryUpdateLogResponseDto> | PageDto<GetDeliveryStatusLogResponseDto>;
  open: boolean;
  loading: boolean;
  onClose: () => void;
...
const updateLogNameMap: Record<string, string> = {
  ...
  weight: "무게",
  message: "메시지",
  collection_site_id: "수거지 ID",
  recipient_name: "수취인 이름",
  recipient_phone: "수취인 전화번호",
  recipient_zipcode: "수취인 우편번호",
  recipient_address: "수취인 주소",
  recipient_address_detail: "수취인 상세 주소",
}

function DeliveryLogsModal({ log, logType, open, loading, onClose, onChangePage }: Props) {
  const size = 20;
  const sort = "desc";
  ...
  const getTitle = () => {
    return logType === 'update' ? '배송 수정 이력' : '배송 상태 변경 이력';
  };

  const updateLogs = log as PageDto<GetDeliveryUpdateLogResponseDto>;
  const statusLogs = log as PageDto<GetDeliveryStatusLogResponseDto>;
                  ...
                  {logType === 'update' ? (
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell align="center">배송 ID</TableCell>
