...
interface Props {
  ...
  loading: boolean;
  onClose: () => void;
  onConfirm: (allowanceTypeId: number, dto: UpdateAllowanceTypeRequestDto) => void;
}
...
function UpdateAllowanceTypeModal({ allowanceType, open, loading, onClose, onConfirm }: Props) {
  const initialForm = { code: allowanceType?.code, name: "", description: "", isActive: true };
  ...
  useEffect(() => {
    ...
    if (allowanceType) {
      setForm({
        ...
        isActive: allowanceType.active ?? true
      });
    } else {
      setForm(initialForm);
    }
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (isNameEmpty || loading || !allowanceType) return;
  ...
  const handleConfirmUpdate = () => {
    ...
    const dto: UpdateAllowanceTypeRequestDto = form;
    onConfirm(allowanceType.id, dto);
  ...
  return (
    ...
    >
      ...
      <DialogActions>
        <Button type="submit" form="updateAllowanceTypeForm" disabled={loading || isNameEmpty}>
          완료
        </Button>
      </DialogActions>
...
export default UpdateAllowanceTypeModal;